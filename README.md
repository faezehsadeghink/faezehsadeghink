<div align="center">

# Hi, I'm Faezeh Sadeghi Niestanak 👋

### Remote Sensing Researcher · M.Sc. Student at the University of Tehran

*Exploring Earth observation through satellite imagery, machine learning, and geospatial data analysis.*

[![Profile](https://img.shields.io/badge/GitHub-faezehsadeghink-181717?style=for-the-badge&logo=github)](https://github.com/faezehsadeghink)
![Remote Sensing](https://img.shields.io/badge/Remote%20Sensing-2E8B57?style=for-the-badge)
![Earth Observation](https://img.shields.io/badge/Earth%20Observation-1565C0?style=for-the-badge)
![Satellite Image Processing](https://img.shields.io/badge/Satellite%20Image%20Processing-6A1B9A?style=for-the-badge)

</div>

## About Me

I am a **Remote Sensing M.Sc. student at the University of Tehran**, interested in using satellite observations to study environmental and Earth-surface processes.

My research interests include machine learning and deep learning for Earth observation, satellite image processing, change detection, satellite hydrology, and large-scale geospatial data analysis. My current research focuses on applying machine learning to predict **GRACE water-storage anomalies**.

Through my academic projects, I have worked with optical and SAR satellite imagery on topics such as land-cover classification, land-surface temperature, geometric correction and orthorectification, InSAR-based land-subsidence analysis, and web-based visualization of spatial data.

## Research Interests

- Remote Sensing and GIS
- Machine Learning and Deep Learning for Earth Observation
- Satellite Image Processing
- Satellite Hydrology and GRACE Data
- Change Detection
- SAR Interferometry and Land-Subsidence Monitoring
- Large-Scale Geospatial Data Processing


## Tools & Technologies

### Programming & Deep Learning

![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![MATLAB](https://img.shields.io/badge/MATLAB-EF6C00?style=flat-square)
![C++](https://img.shields.io/badge/C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat-square&logo=pytorch&logoColor=white)
![Ultralytics YOLO](https://img.shields.io/badge/Ultralytics%20YOLO-111F68?style=flat-square)

### Remote Sensing & Geospatial Analysis

![ENVI](https://img.shields.io/badge/ENVI-3B6E8F?style=flat-square)
![SNAP](https://img.shields.io/badge/SNAP-0B4F8A?style=flat-square)
![Google Earth Engine](https://img.shields.io/badge/Google%20Earth%20Engine-4285F4?style=flat-square&logo=googleearthengine&logoColor=white)
![ArcGIS](https://img.shields.io/badge/ArcGIS-2C7AC3?style=flat-square)
![Global Mapper](https://img.shields.io/badge/Global%20Mapper-376B44?style=flat-square)
![Agisoft Metashape](https://img.shields.io/badge/Agisoft%20Metashape-5D6D7E?style=flat-square)

### Web Visualization & Spatial Databases

![GeoServer](https://img.shields.io/badge/GeoServer-4F9B45?style=flat-square)
![OpenLayers](https://img.shields.io/badge/OpenLayers-1F6B75?style=flat-square&logo=openlayers&logoColor=white)
![Cesium](https://img.shields.io/badge/CesiumJS-6CADDF?style=flat-square&logo=cesium&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![PostGIS](https://img.shields.io/badge/PostGIS-336791?style=flat-square)
![FME](https://img.shields.io/badge/FME-ED1C24?style=flat-square)

## Selected Projects

### 🌐 [Web-Based Geovisualization Projects](https://github.com/faezehsadeghink/web-geovisualization-projects)

A four-stage collection covering 2D web mapping with GeoServer and OpenLayers, X3D visualization, Cesium/3D Tiles workflows, and location-based WebAR.

**Live demo:** [faezehsadeghink.github.io/web-geovisualization-projects](https://faezehsadeghink.github.io/web-geovisualization-projects/)

`OpenLayers` `GeoServer` `X3D` `CesiumJS` `FME` `Leaflet` `A-Frame` `AR.js`

---

### 🛰️ [Satellite Image Geometric Correction](https://github.com/faezehsadeghink/satellite-image-geometric-correction)

Geometric image rectification, DEM-assisted orthorectification, and QuickBird-2 RPC/RFM geometric accuracy assessment.

`Python` `SPOT` `QuickBird-2` `RPC` `RFM` `Orthorectification`

---

### 🐍 [Python Satellite Imagery Analysis](https://github.com/faezehsadeghink/python-satellite-imagery-analysis)

Landsat 9 analysis covering spectral indices, emissivity and land-surface temperature, and supervised land-cover classification with accuracy and Kappa assessment.

`Python` `Landsat 9` `NDVI` `LST` `Remote Sensing` `Image Classification`

---

### 💻 [C++ Console Projects](https://github.com/faezehsadeghink/cpp-console-projects)

Two foundational C++ projects: an arbitrary-length integer calculator and a terminal-based Pac-Man game with multiple maps, ghosts, scoring, and difficulty modes.

`C++17` `Algorithms` `Console Game` `CMake` `Big Integer`

## Current Focus

- Machine learning for predicting GRACE terrestrial water-storage anomalies
- Deep learning for Earth-observation imagery
- Change detection and satellite-image processing
- Reproducible remote-sensing workflows

## Contact

You can follow my work or connect with me through:

- GitHub: [@faezehsadeghink](https://github.com/faezehsadeghink)
- LinkedIn: [Faezeh Sadeghi](https://linkedin.com/in/faezeh-sadeghi-2256a5288)

---

<div align="center">

*More remote-sensing, Earth-observation, and image-processing projects are coming soon.*

</div>
